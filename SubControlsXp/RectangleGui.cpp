...
Color FromHexStringBackwardCompatible(const std::wstring &s)
{
	...
	//return Color(se_sdk::FastGamma::srgbToLinear(static_cast<float>((hex >> 16) & 0xff) * oneOver255),
	//	se_sdk::FastGamma::srgbToLinear(static_cast<float>((hex >> 8) & 0xff) * oneOver255),
	//	se_sdk::FastGamma::srgbToLinear(static_cast<float>(hex & 0xff) * oneOver255), alpha);
//	return Color(se_sdk::FastGamma::srgbToLinear((hex >> 16) & 0xff), se_sdk::FastGamma::sRGB_to_float((hex >> 8) & 0xff), se_sdk::FastGamma::sRGB_to_float(hex & 0xff), alpha);
	//	return Color(static_cast<float>((hex >> 16) & 0xff) * oneOver255, static_cast<float>((hex >> 8) & 0xff) * oneOver255, static_cast<float>(hex & 0xff) * oneOver255, alpha);
...
/*
	...
	{
		drawTestNext = false;
		auto bb = g.CreateSolidColorBrush(Color::Red);
		g.FillRectangle(r, bb);
		return 0;
	}
*/
