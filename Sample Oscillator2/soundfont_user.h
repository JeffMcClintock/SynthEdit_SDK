...
public:
	soundfontUser(void);
	virtual ~soundfontUser(void);
	void GetZone(short p_chan, short p_note, short p_vel);
//	inline float do_interpolation(short* sample_ptr, int fine );
//	inline float do_interpolation_linear(short* sample_ptr, int fine ); // inline works without definition???!!!
	activeZoneListType playingZones;
