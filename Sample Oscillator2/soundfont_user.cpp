...
soundfontUser::soundfontUser(void) :
sampleHandle(-1)
{
}
...
#define DEBUG_ZONES
	...
	SampleManager::Instance()->GetPlayingZones( sampleHandle, p_chan, p_note, p_vel, playingZones );

	partials.resize( playingZones.size() );
	...
	for( activeZoneListType::iterator it = playingZones.begin() ; it != playingZones.end() ; ++it )
	{
		...
		partial.cur_sample_r = SampleManager::Instance()->GetSampleHeader( sampleHandle, z.Get(53).wAmount );
...
void partial::SetPan( float pan ) // 0.5 = hard right. -0.5 = hard left.
{
	...
	//MessageBox( 0, msg, L"",MB_OK|MB_ICONSTOP );
